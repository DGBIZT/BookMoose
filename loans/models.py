...
class BookLoan(models.Model):
    ...
    is_returned = models.BooleanField(
        ...
        help_text='Отметка о возврате'
    )
    due_date = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name='Срок возврата',
        help_text='Планируемая дата возврата'
    )
    notes = models.TextField(
        blank=True,
        verbose_name='Примечания',
        help_text='Дополнительные комментарии (например, повреждения)'
    )
    ...
    def __str__(self):
        return (
            f"{self.book_instance.copy_id} → {self.user.username} "
            f"({self.loan_date.strftime('%d.%m.%Y')})"
        )
