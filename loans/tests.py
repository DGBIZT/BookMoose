...
class BookLoanViewSetTest(APITestCase):
    ...
    def test_user_can_list_own_loans(self):
        ...
        first_loan = BookLoan.objects.create(
            book_instance=self.book_instance,
            user=self.user,
    ...
    def test_staff_can_list_all_loans(self):
        ...
        book_instance_1 = BookInstance.objects.create(
            book=self.book,
            status='available',
            copy_id='COPY001'  # ← обязательно укажите
        )
        book_instance_2 = BookInstance.objects.create(
            book=self.book,
            status='available',
            copy_id='COPY002'  # ← обязательно укажите
        )
    ...
    def test_user_can_update_own_loan_notes_and_due_date(self):
        ...
        data = {
            'notes': 'Updated notes',
            'due_date': (timezone.now() + datetime.timedelta(days=21)).isoformat()
        }
        self.client.force_authenticate(user=self.user)
    ...
    def test_ordering_by_loan_date(self):
        ...
        book_instance1 = BookInstance.objects.create(
            book=self.book,
            copy_id='COPY001'  # ← обязательное поле
        )
        book_instance2 = BookInstance.objects.create(
            book=self.book,
            copy_id='COPY002'  # ← обязательное поле
        )

        loan1 = BookLoan.objects.create(
            book_instance=book_instance1,
            user=self.user,
            loan_date=timezone.now() - datetime.timedelta(days=7)
        )
        loan2 = BookLoan.objects.create(
            book_instance=book_instance2,
            user=self.user,
            loan_date=timezone.now()
        )
        self.client.force_authenticate(user=self.staff_user)
    ...
    def test_pagination(self):
        ...
        for i in range(25):
            book_instance = BookInstance.objects.create(
                book=self.book,
                copy_id=f'COPY{i:03d}'  # Уникальное значение
            )
    ...
    def test_get_detail_for_other_loan_denied(self):
        ...
        other_user = User.objects.create_user(
            username='other',
            password='otherpass',
            email='other@example.com'  # ← обязательно укажите
        )
