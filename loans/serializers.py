...
class BookLoanSerializer(serializers.ModelSerializer):
    ...
    copy_id = serializers.CharField(source="book_instance.copy_id", read_only=True)
    ...
    def validate(self, data):
        ...
        if not self.instance:
            if not book_instance:
                raise serializers.ValidationError({"book_instance": "Обязательно укажите экземпляр книги."})
            if book_instance.status != "available":
                raise serializers.ValidationError({"book_instance": "Экземпляр уже выдан или недоступен."})
