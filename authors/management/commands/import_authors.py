...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'excel_file',
            type=str,
            help='Путь к Excel‑файлу (.xlsx) с авторами'
        )
        parser.add_argument(
            '--user-id',
            type=int,
            default=1,
            help='ID пользователя, который создаст записи (created_by)'
        )
        ...
        try:
            ...
            if missing_required:
                self.stderr.write(self.style.ERROR(
                    f"Ошибка: Не найдены обязательные поля: {', '.join(missing_required)}"
                ))
                sys.exit(1)
            ...
            for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
                ...
                try:
                    ...
                    middle_name = self.clean_string(row[col_indices.get('middle_name')].value) if 'middle_name' in col_indices else None
                    biography = self.clean_string(row[col_indices.get('biography')].value) if 'biography' in col_indices else None
                    ...
                    if 'birth_date' in col_indices and row[col_indices['birth_date']].value:
                        try:
                            ...
                            self.stderr.write(self.style.WARNING(
                                f"Строка {row_idx}: некорректная дата рождения '{row[col_indices['birth_date']].value}'. Пропущено."
                            ))
                            continue
                    ...
                    if 'death_date' in col_indices and row[col_indices['death_date']].value:
                        try:
                            ...
                            self.stderr.write(self.style.WARNING(
                                f"Строка {row_idx}: некорректная дата смерти '{row[col_indices['death_date']].value}'. Пропущено."
                            ))
                            continue
                    ...
                    if not last_name:
                        self.stderr.write(self.style.WARNING(
                            f"Строка {row_idx}: пропущена — отсутствует фамилия (last_name)."
                        ))
                        error_count += 1
                        continue
                    ...
                    if not first_name:
                        self.stderr.write(self.style.WARNING(
                            f"Строка {row_idx}: пропущена — отсутствует имя (first_name)."
                        ))
                        error_count += 1
                        continue
