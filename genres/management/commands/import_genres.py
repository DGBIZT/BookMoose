...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("excel_file", type=str, help="Путь к Excel‑файлу (.xlsx) с жанрами")
        parser.add_argument(
            "--user-id", type=int, default=1, help="ID пользователя, который создаст жанры (created_by)"
        )
        ...
        try:
            ...
            for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
                try:
                    ...
                    description = (
                        str(row[desc_idx].value or "").strip().replace("\xa0", " ").strip() if desc_idx != -1 else ""
                    )
                    order_str = (
                        str(row[order_idx].value or "0").strip().replace("\xa0", " ").strip()
                        if order_idx != -1
                        else "0"
                    )
                    parent_name = (
                        str(row[parent_idx].value or "").strip().replace("\xa0", " ").strip()
                        if parent_idx != -1
                        else ""
                    )
                    ...
                    if not name:
                        self.stderr.write(
                            self.style.WARNING(f"Строка {row_idx}: пропущена — отсутствует поле 'name'.")
                        )
                        error_count += 1
                        continue
                    ...
                    if not order_str.isdigit():
                        self.stderr.write(
                            self.style.WARNING(f"Строка {row_idx}: поле 'order' не число: {order_str}. Пропущено.")
                        )
                        error_count += 1
                        continue
                    order = int(order_str)
                    ...
                    if parent_name:  # Только если parent_name не пустой
                        try:
                            ...
                            self.stderr.write(
                                self.style.WARNING(
                                    f"Строка {row_idx}: родительский жанр "
                                    f"'{parent_name}' не найден. Будет без родителя."
                                )
                            )
