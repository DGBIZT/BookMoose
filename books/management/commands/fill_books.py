...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("file_path", type=str, help="Путь к Excel-файлу (XLSX или XLS)")
        parser.add_argument(
            "--user-id",
            type=int,
            default=None,
    ...
    def handle(self, *args, **options):
        ...
        for row in data:
            try:
                ...
                if not authors:
                    self.stderr.write(
                        self.style.WARNING(f'Нет авторов для книги "{row.get("title", "без названия")}". Пропускаем.')
                    )
                    continue
                    ...
                    for copy_id in copy_ids:
                        try:
                            ...
                            if BookInstance.objects.filter(book=book, copy_id=copy_id).exists():
                                self.stderr.write(
                                    self.style.WARNING(
                                        f'Экземпляр с copy_id="{copy_id}" для книги '
                                        f'"{book.title}" уже существует. Пропускаем.'
                                    )
                                )
                                continue
                            ...
                            instance.save()
                            created_instances += 1
                            self.stdout.write(self.style.SUCCESS(f"Создан экземпляр: {copy_id} (книга: {book.title})"))
                        except ValidationError as e:
                            errors.append(f'Ошибка валидации экземпляра "{copy_id}" книги "{book.title}": {e}')
                        except Exception as e:
                            errors.append(f'Ошибка при создании экземпляра "{copy_id}" книги "{book.title}": {e}')
            ...
            if file_path.endswith(".xlsx"):
                ...
                for row_idx in range(1, sheet.nrows):
                    ...
                    if missing_fields:
                        self.stderr.write(
                            self.style.WARNING(
                                f"Строка {row_idx + 1}: пропущена из‑за незаполненных полей: "
        ...
        for name in author_names:
            ...
            try:
                author, created = Author.objects.get_or_create(
                    last_name=last_name, first_name=first_name, middle_name=middle_name or None, created_by=self.user
                )
    ...
    def get_genre(self, genre_name):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'Ошибка при создании жанра "{name}": {e}'))
            return None
