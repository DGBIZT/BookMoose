...
class BookModelTest(TestCase):
    def setUp(self):
        ...
        self.author = Author.objects.create(
            last_name="Тестов", first_name="Иван", created_by=self.user  # обязательное поле created_by
        )

    def test_book_creation(self):
        # Создаём книгу
        book = Book.objects.create(
            title="Тест-книга", created_by=self.user, publication_year=2020, pages=300, language="русский"
        )
        ...
        self.assertEqual(str(book), "Тест-книга (Иван Тестов, 2020)")

    def test_isbn_unique(self):
        Book.objects.create(
            title="Книга 1", created_by=self.user, isbn="123-456-789", publication_year=2020, pages=100
        )
        with self.assertRaises(ValidationError):
            book2 = Book(
                title="Книга 2", created_by=self.user, isbn="123-456-789", publication_year=2021, pages=150  # Дубликат
            )
            book2.full_clean()

    def test_publication_year_range(self):
        with self.assertRaises(ValidationError):
            book = Book(title="Старая книга", created_by=self.user, publication_year=1400, pages=100)  # < 1450
            book.full_clean()

        with self.assertRaises(ValidationError):
            book = Book(title="Будущая книга", created_by=self.user, publication_year=3000, pages=100)  # > 2030
            book.full_clean()
...
class BookInstanceModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="testuser", password="pass123")
        self.book = Book.objects.create(title="Основная книга", created_by=self.user, publication_year=2020, pages=200)

    def test_instance_creation(self):
        instance = BookInstance.objects.create(book=self.book, copy_id="COPY-001", status="available")
...
class BookSerializerTest(APITestCase):
    def setUp(self):
        ...
        self.author = Author.objects.create(
            ...
            first_name="Автор",
            created_by=User.objects.create_user(username="serializer_test_user", password="testpass"),
        )
    ...
    def test_serializer_invalid_data(self):
        data = {"title": "", "publication_year": 1000, "pages": -1}
        serializer = BookSerializer(data=data)
...
class BookInstanceSerializerTest(APITestCase):
    def setUp(self):
        ...
        self.author = Author.objects.create(last_name="Тестов", first_name="Иван", created_by=self.user)
...
class BookViewSetTest(APITestCase):
    ...
    def test_delete_book_with_loaned_copies(self):
        ...
        BookInstance.objects.create(book=self.book, copy_id="LOANED-001", status="loaned")
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(reverse("books:book-detail", kwargs={"pk": self.book.pk}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_add_copies_action(self):
        ...
        self.assertEqual(BookInstance.objects.filter(book=self.book).count(), 3)
    ...
    def test_filter_books_by_author(self):
        ...
        response = self.client.get(reverse("books:book-list"), query_dict)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        titles = [item["title"] for item in response.data["results"]]
        self.assertIn(self.book.title, titles)

    def test_search_books_by_title(self):
        response = self.client.get(reverse("books:book-list"), {"search": "Тестовая"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class PublicBookViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.book = Book.objects.create(
            title="Публичная книга", publication_year=2021, pages=150, created_by=self.user  # Обязательное поле!
        )
...
class BookInstanceViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="borrower", password="pass123", email="borrower@example.com", is_staff=True
        )
        self.book = Book.objects.create(
            title="Книга для экземпляра", publication_year=2020, pages=100, created_by=self.user
        )
        self.instance = BookInstance.objects.create(book=self.book, copy_id="INSTANCE-001", status="available")
    ...
    def test_update_instance_status(self):
        ...
        response = self.client.patch(
            reverse("books:instance-detail", kwargs={"pk": self.instance.pk}), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_instance(self):
        ...
        response = self.client.get(reverse("books:instance-detail", kwargs={"pk": self.instance.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["copy_id"], "INSTANCE-001")

    def test_filter_instances_by_book(self):
        response = self.client.get(reverse("books:instance-list"), {"book": self.book.pk})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        copy_ids = [item["copy_id"] for item in response.data]
        self.assertIn("INSTANCE-001", copy_ids)

    def test_filter_instances_by_status(self):
        response = self.client.get(reverse("books:instance-list"), {"status": "available"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_filter_instances_by_borrower(self):
        ...
        response = self.client.get(reverse("books:instance-list"), {"borrower": self.user.pk})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_create_instance_as_staff_success(self):
        ...
        data = {"book": self.book.pk, "copy_id": "NEW-001", "status": "available"}

        response = self.client.post(reverse("books:instance-list"), data, format="json")
    ...
    def test_acquisition_date_can_be_null(self):
        ...
        # Проверяем статус
        self.assertEqual(
            response.status_code, status.HTTP_200_OK, "Должен возвращаться 200 OK для авторизованного пользователя"
        )

        # Проверяем, что acquisition_date == null
        self.assertIsNone(response.data["acquisition_date"], "acquisition_date должен быть null по умолчанию")
...
class UtilityTests(TestCase):
    def test_book_str_method(self):
        ...
        book = Book.objects.create(title="Название", created_by=user, publication_year=2000, pages=100)
    ...
    def test_bookinstance_str_method(self):
        ...
        book = Book.objects.create(title="Книга", publication_year=2020, pages=50, created_by=user)  # ← Обязательно!

        # Создаём экземпляр книги
        instance = BookInstance.objects.create(book=book, copy_id="ID-123", status="damaged")
    ...
    def test_book_update_copies_count(self):
        ...
        book = Book.objects.create(title="Счётчик", created_by=user, publication_year=2021, pages=200)
    ...
    def test_bookinstance_set_status(self):
        ...
        # Создаём книгу с указанием created_by
        book = Book.objects.create(title="Тест", publication_year=2022, pages=100, created_by=user)  # ← Обязательно!

        # Создаём экземпляр книги
        instance = BookInstance.objects.create(book=book, copy_id="TEST-01", status="available")
